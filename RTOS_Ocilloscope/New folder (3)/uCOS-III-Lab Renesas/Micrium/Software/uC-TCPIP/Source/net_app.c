...
*
* Description : (1) Connect a client to a server using his host name with a stream (TCP) socket
                                                       ...
                                                       NET_ERR                  *p_err)
{
    ...
    while (done == DEF_NO) {
     ...
     defined(NET_IPv6_MODULE_EN))
                case NET_SOCK_ERR_INVALID_ADDR:
                case NET_APP_ERR_CONN_FAIL:                     /* If the connection fail, the                          */
                                                         ...
                                                         NET_ERR             *p_err)
{
    ...
    if (do_dns == DEF_YES) {
        ...
        status = DNSc_GetHost(p_remote_host_name, &addr_dns, &addr_nbr, flags, DEF_NULL, &err_dns);
        switch (status) {
            case DNSc_STATUS_RESOLVED:
                 if (addr_nbr != 0) {
                     Mem_Copy(addr, addr_dns.Addr, addr_dns.Len);
