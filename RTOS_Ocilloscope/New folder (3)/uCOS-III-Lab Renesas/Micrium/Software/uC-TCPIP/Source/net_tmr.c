...
void  NetTmr_TaskHandler (void)
{
    ...
    while (NetTmr_TaskListPtr != DEF_NULL) {                    /* Handle  Tmr Task List tmrs (see Note #5a).           */
...
*
*               (6) See 'NetTmr_TaskHandler()  Note #2b'.
*********************************************************************************************************
*/
                      ...
                      NET_ERR        *p_err)
{
   ...
   (void)&flags;                                                /* Prevent 'variable unused' warning (see Note #5).     */
    ...
    NetTmr_TaskListHead = p_tmr;                                /* Insert tmr @ list head (see Note #6).                */
...
static  void  NetTmr_DiscardTaskTmr (void)
{
    ...
    p_tmr      = (NET_TMR *)NetTmr_TaskListHead;
    p_tmr_prev =  DEF_NULL;
    while ((p_tmr != DEF_NULL) &&                               /* Srch entire Tmr Task List ...                        */
           (p_tmr != (NET_TMR *)NetTmr_TaskListPtr)) {          /* ... for corrupted tmr task ptr (see Note #2a).       */
        p_tmr_prev = (NET_TMR *)p_tmr;
        p_tmr      = (NET_TMR *)p_tmr->NextPtr;
    }
                                                                /* Unlink corrupted tmr task ptr from Tmr Task List :   */
    if (p_tmr_prev != DEF_NULL) {                               /* If NOT @ head of Tmr Task List, ...                  */
        p_tmr_prev->NextPtr  = DEF_NULL;                        /* ...  clr prev tmr's next tmr.                        */
    } else {                                                    /* Else clr Tmr Task List.                              */
        NetTmr_TaskListHead = DEF_NULL;
    }

    NetTmr_TaskListPtr = DEF_NULL;                              /* Clr Tmr Task ptr.                                    */
}
#endif
#endif
