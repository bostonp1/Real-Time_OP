...
CPU_BOOLEAN  NetARP_CfgReqMaxRetries (CPU_INT08U  max_nbr_retries)
{
    ...
    CPU_CRITICAL_ENTER();
    NetARP_ReqMaxAttempts_nbr = max_nbr_retries + 1u;       /* Set max attempts as max retries + 1 (see Note #1).       */
    CPU_CRITICAL_EXIT();
                           ...
                           NET_ERR  *p_err)
{
    ...
    if (p_cache_addr_arp != (NET_CACHE_ADDR_ARP *)0) {          /* If ARP cache found, chk state.                       */
        switch (*p_err) {
            case NET_CACHE_ERR_PEND:                            /* If ARP cache pend, append buf into Q (see Note #4a1).*/
            ...
            case NET_CACHE_ERR_RESOLVED:                        /* If ARP cache resolved, copy hw addr.                 */
                 Mem_Copy(p_addr_hw,
        ...
        if (addr_protocol_multicast == DEF_YES) {               /* If multicast protocol addr,      ...                 */
            ...
            NetCache_AddResolved(if_nbr, p_addr_hw, p_addr_protocol, NET_CACHE_TYPE_ARP, &err);
...
void  NetARP_CacheTimeout (void  *p_cache_timeout)
{
    ...
    if (p_cache == (NET_ARP_CACHE *)0) {
        ...
        return;
                                                                ...
                                                                /* ---------------- DISCARD ARP CACHE ----------------- */
    NetCache_Remove((NET_CACHE_ADDR *)p_cache->CacheAddrPtr,    /* Clr but do NOT free tmr (see Note #3).               */
                                      DEF_NO);
}
                                       ...
                                       NET_ERR      *p_err)
{
    ...
    if (p_cache_addr_arp != (NET_CACHE_ADDR_ARP *)0) {          /* If ARP cache found, chk state.                       */
        p_cache_arp = (NET_ARP_CACHE *)NetCache_GetParent((NET_CACHE_ADDR *)p_cache_addr_arp);
        switch (*p_err) {
            case NET_CACHE_ERR_PEND:                            /* If ARP cache pend, add sender's hw addr, ...         */
            ...
            case NET_CACHE_ERR_RESOLVED:                        /* If ARP cache resolved, update sender's hw addr.      */
                 Mem_Copy((void     *)&p_cache_addr_arp->AddrHW[0],
                          (void     *) p_addr_sender_hw,
                          (CPU_SIZE_T) NET_IF_HW_ADDR_LEN_MAX);
                                                                /* Reset ARP cache tmr (see Note #4).                   */
            ...
            default:
                 NetCache_Remove((NET_CACHE_ADDR *)p_cache_addr_arp, DEF_YES);
                 NetCache_AddResolved(if_nbr, p_addr_sender_hw, p_addr_sender_protocol, NET_CACHE_TYPE_ARP, p_err);
                 break;
        }
...
static  void  NetARP_CacheReqTimeout (void  *p_cache_timeout)
{
    ...
    if (p_cache->ReqAttemptsCtr >= th_max) {                    /* If nbr attempts >= max, free ARP cache.              */
        NetCache_Remove((NET_CACHE_ADDR *)p_cache_addr_arp,     /* Clr but do NOT free tmr (see Note #2).               */
                                          DEF_NO);
