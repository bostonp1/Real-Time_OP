...
const  NET_PHY_API_ETHER  NetPhy_API_DP83640 = { NetPhy_Init,                       /*   Init                           */
                                                 NetPhy_EnDis,                      /*   En/dis                         */
                           ...
                           NET_ERR  *perr)
{
    ...
    if (pphy_cfg->BusMode == NET_PHY_BUS_MODE_RMII) {           /* ------------- CONFIG RMII MASTER MODE -------------- */
                                                                /* Sel page 6, PTP clock output control reg #2.         */
        pdev_api->Phy_RegWr(pif, phy_addr, PHY_PAGESEL, PAGESEL_PTP_CFG2, perr);
        if (*perr != NET_PHY_ERR_NONE) {
             return;
        }
                                                                /* Rd ctrl reg, get PTP clock output en bit.            */
        pdev_api->Phy_RegRd(pif, phy_addr, PHY_PTP_COC, &reg_val, perr);
        if (*perr != NET_PHY_ERR_NONE) {
             return;
        }

        reg_val &= ~PTP_CLKOUT_EN;                              /* Disable PTP clock output.                            */
        pdev_api->Phy_RegWr(pif, phy_addr, PHY_PTP_COC, reg_val, perr);
        if (*perr != NET_PHY_ERR_NONE) {
             return;
        }
                                                                /* Sel page 0, extended reg.                            */
        pdev_api->Phy_RegWr(pif, phy_addr, PHY_PAGESEL, PAGESEL_EXT_REG, perr);
        if (*perr != NET_PHY_ERR_NONE) {
             return;
        }
    }
