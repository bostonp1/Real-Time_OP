...
*                                          HTTPsResp_Prepare()
*
* Description : (1) Prepare server response
...
*
* Return(s)   : DEF_YES, if preparation is done.
*               DEF_NO,  otherwise.
*
                                ...
                                HTTPs_CONN      *p_conn)
{
    ...
    if ((p_conn->Method           != HTTP_METHOD_HEAD         ) &&
        ...
        DEF_BIT_CLR(p_conn->Flags, HTTPs_FLAG_RESP_BODY_PRESENT);
    }
...
*                                           HTTPsResp_Handle()
*
* Description : (1) Prepare connection to transmit the request response:
*
*                   (a) Get file content type and update response state accordingly.
*                   (b) Parse file token and get their values.
*                   (b) Prepare response:
*                       (i)   Prepare response line.
*                       (ii)  Prepare response header section.
*                       (iii) Update connection parameters to transmit response.
*                       (iv)  Update response state to transmit file or to close the connection.
*
*                   (c) Transmit file:
*                       (i)   Read file.
*                       (ii)  Parse and replace token from the connection buffer.
*                       (iii) Validate that the file is completely transmitted.
*
*                   (d) Close file.
*
* Argument(s) : p_instance  Pointer to the instance.
...
*               ------      Argument validated in HTTPsSock_ConnAccept().
*
* Return(s)   : DEF_YES, if response handling is completed.
*               DEF_NO,  otherwise.
*
* Caller(s)   : HTTPsConn_Process().
*
    ...
    switch (p_conn->State) {
        ...
        case HTTPs_CONN_STATE_RESP_DATA_CHUNCKED_TX_LAST_CHUNK:
             ...
             switch (err) {
                 ...
                 case HTTPs_ERR_TOKEN_POOL_EMPTY:
                      p_conn->SockState = HTTPs_SOCK_STATE_NONE;
                      break;
...
*
* Description : (1) Terminate the body data stage:
...
*
* Description : (1) Prepare connection following the status code
...
*               ------      Argument validated in HTTPsSock_ConnAccept().
*
* Return(s)   : DEF_YES, if status code is related to an error.
*               DEF_NO,  otherwise.
*
* Caller(s)   : HTTPsResp_Prepare().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Description : Open file for the connection, if required.
...
*               ------      Argument validated in HTTPsSock_ConnAccept().
*
* Return(s)   : DEF_OK    if the Body Data is ready to be processed.
*               DEF_FAIL  otherwise.
*
* Caller(s)   : HTTPsResp_Prepare().
*
...
*
* Description : (1) Prepare response status line:
*
...
*
*                           HTTPs_ERR_NONE                          Status line     successfully prepared.
*                           HTTPs_ERR_RESP_STATUS_LINE              Status line NOT successfully prepared.
*
* Return(s)   : none.
*
* Caller(s)   : HTTPs_Resp().
*
* Note(s)     : (2) Connection buffer size must be configured to handle at least the entire status line.
*
*               (3) RFC #2616, Section 6 'Response' specifies how a response message must be structured:
*
...
*
*               (4) RFC #2616, Section 6.1 'Status Line' specifies how the response status line message must be
*                   structured:
*
*                   (a) 6.1 Status-Line
*
                                    ...
                                    HTTPs_ERR       *p_err)
{
   ...
   (void)&p_instance;                                           /* Prevent possible 'variable unused' warnings.         */
...
* Caller(s)   : HTTPsResp_Handle().
*
* Note(s)     : (2) RFC #2616, Section 6 'Response' specifies how a response message must be structured:
*
*                   (a) After receiving and interpreting a request message, a server responds with an HTTP
*                       response message.
*
...
*                                           [ message-body ]          ; Section 7.2
*
*               (3) RFC #2616, Section 6.2 'Response Header Fields' specifies how the response header fields
...
*
* Description : (1) Add response header field:
*
...
*               ------          Argument validated in HTTPsSock_ConnAccept().
*
*               p_buf           Pointer to the buffer where to copy header field.
*               -----           Argument validated in HTTPsResp_Hdr().
*
*               buf_len         Remaining length available in the buffer.
*
*               field_type      Header field type to add.
*
*               p_val           Pointer to value of header when an Hdr Fields list is present.
*
*               val_len         Length of the value.
*
* Return(s)   : Pointer to the end of the header field added, if field successfully copied.
*
*               DEF_NULL,                                     otherwise.
*
* Caller(s)   : HTTPsResp_Hdr().
*
* Note(s)     : (2) RFC #2616, Section 6 'Response' specifies how a response message must be structured:
*
*                   (a) After receiving and interpreting a request message, a server responds with an HTTP
...
*
* Description : Transfer message-body data with the standard way i.e with the Content-Length header and
*               no body encoding.
*
* Argument(s) : p_instance  Pointer to the instance.
*               ----------  Argument validated in HTTPs_InstanceStart().
...
*               ------      Argument validated in HTTPsSock_ConnAccept().
*
* Return(s)   : DEF_YES, if all data has been read.
*               DEF_NO,  if data still need to be read.
*
* Caller(s)   : HTTPsResp_Handle().
*
* Note(s)     : (1) RFC #2616, Section 6 'Response' specifies how a response message must be structured:
*
*                   (a) After receiving and interpreting a request message, a server responds with an HTTP
*                       response message.
*
...
*
* Description : Transfers the message-body using chunked transfer coding.
...
*
* Description : Encapsulate the data into the chunked encoding format.
...
*
*
* Argument(s) : p_buf           Pointer to start of buffer.
*
*               data_len        Length of the data inside the buffer
*
*               len_dig_str_len Length of the hex formatted string length.
*
* Return(s)   : Return the length of the the encoded data.
*
* Caller(s)   : HTTPsResp_DataTransferChunkedWithHook().
*
* Note(s)     : None.
*********************************************************************************************************
*/
...
*
* Description : Transfer message-body (html & text document) using chunked transfer encoding.
*
* Argument(s) : p_instance  Pointer to the instance.
*               ----------  Argument validated in HTTPs_InstanceStart().
*
...
*                                           [ message-body ]          ; Section 7.2
*
*               (2) RFC #2616, Section 3.631 'Chunked Transfer Coding' specifies how a message-body must
*                   be structured:
*
                                                    ...
                                                    HTTPs_ERR       *p_err)
{
    const  HTTPs_CFG             *p_cfg;
           ...
           CPU_CHAR              *p_wr;
           HTTPs_INSTANCE_ERRS   *p_ctr_err;
           HTTPs_INSTANCE_STATS  *p_ctr_stat;
    ...
    switch (p_conn->State) {
        case HTTPs_CONN_STATE_RESP_DATA_CHUNCKED:
             ...
             if (p_conn->TokenBufRemLen == 0u) {
                 ...
                 (void)HTTPsMem_TokenGet(p_instance, p_conn, p_err);
                 switch (*p_err) {
                     case HTTPs_ERR_NONE:
                          break;
...
*
* Description : Search for token to be parsed or replaced in a sting buffer.
*
* Argument(s) : p_buf               Pointer to the buffer that contains the string to search within.
*
*               buf_len             Length of the buffer.
*
*               p_str_token         Pointer to pointer that will set to the beginning of the token found.
*
*               p_err   Pointer to variable that will receive the return error code from this function :
*
*                           HTTPs_ERR_NONE                              Token successfully found.
*
*                           HTTPs_ERR_TOKEN_NO_TOKEN_FOUND              No token found in the buffer.
*                           HTTPs_ERR_TOKEN_MORE_DATA_REQ               A token may be found but can not be validated,
*                                                                           more data is required to complete the search.
*
* Return(s)   : Length of token found (including start char and end char of the token), if token found and validated.
*
*               0,                                                                      otherwise.
*
* Caller(s)   : HTTPsResp_FileTransferChunked().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Description : Called for each token (${TEXT_STRING}) found in a HTML document.
*
* Argument(s) : p_instance  Pointer to the instance.
*               ----------  Argument validated in HTTPs_InstanceStart().
*
...
*               ------      Argument validated in HTTPsSock_ConnAccept().
*
*               p_val        Pointer to buffer where to copy token value.
*
*               val_len_max  Value buffer length.
*
* Return(s)   : DEF_OK,   if token value copied successfully.
*
*               DEF_FAIL, otherwise (see Note #2).
*
* Caller(s)   : HTTPsResp_DataTransferChunked().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Description : Configure the connection to transmit the static error page.
*
* Argument(s) : p_conn  Pointer to the connection.
*               ------  Argument validated in HTTPsSock_ConnAccept().
*
* Return(s)   : none.
*
* Caller(s)   : HTTPsResp_PrepareStatusCode().
*
* Note(s)     : none.
*********************************************************************************************************
*/
                                        ...
                                        HTTPs_CONN      *p_conn)
{
    ...
    p_conn->DataPtr          = HTTPs_CFG_HTML_DFLT_ERR_PAGE;
...
*                                          HTTPsResp_FileOpen()
*
* Description : Open file from the file system and the size.
*
* Argument(s) : p_instance  Pointer to the instance.
*               ----------  Argument validated in HTTPs_InstanceStart().
*
...
* Return(s)   : none.
*
* Caller(s)   : HTTPsResp_PrepareBodyData().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*               ------          Argument validated in HTTPsSock_ConnAccept().
*
*               p_dst           Pointer to buffer where to store read data.
*               ----            Argument validated in HTTPs_Resp(),
*                                                     HTTPs_TokenValGet().
*
*               dst_len_max     Maximum data that can be put in the destination.
*
* Return(s)   : Length of data read.
*
* Caller(s)   : HTTPsResp_DataTransferChunked(),
*               HTTPsResp_DataTransferStd().
*
